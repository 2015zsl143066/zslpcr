...
Thermocycler::Thermocycler(boolean restarted) :
        ...
        iCycleStartTime(0),
        iRamping(true),
        iPlatePid(&iPlateThermistor.GetTemp(), &iPeltierPwm, &iTargetPlateTemp, PLATE_PID_INC_NORM_P,
                  PLATE_PID_INC_NORM_I, PLATE_PID_INC_NORM_D, DIRECT),
 ...
 /*if(iProgramState == EStartup){
  ...
  //mySerial->write("xixixi! ");
    switch (iProgramState) {
        ...
        case ERunning:
            ...
            if (iProgramState == ERunning) {
                if (iRamping && abs(ipCurrentStep->GetTemp() - GetPlateTemp()) <= CYCLE_START_TOLERANCE &&
                    GetRampElapsedTimeMs() > ipCurrentStep->GetRampDurationS() * 1000) {
                    //begin step hold
                ...
                } else if (!iRamping && !ipCurrentStep->IsFinal() &&
                           millis() - iCycleStartTime > (unsigned long) ipCurrentStep->GetStepDurationS() * 1000) {
                    //begin next step
                    AdvanceToNextStep();
        ...
        case EComplete:
            if (iRamping && ipCurrentStep != NULL &&
...
void Thermocycler::PreprocessProgram() {
    ...
    while ((pCurrentStep = ipProgram->GetNextStep()) && !pCurrentStep->IsFinal()) {
        //validate ramp
        if (pPreviousStep != NULL && pCurrentStep->GetRampDurationS() * 1000 <
                                     absf(pCurrentStep->GetTemp() - pPreviousStep->GetTemp()) *
                                     PLATE_FAST_RAMP_THRESHOLD_MS) {
...
void Thermocycler::UpdateEta() {
    if (iProgramState == ERunning) {
        ...
        unsigned long estimatedDurationS =
                iProgramHoldDurationS + iProgramControlledRampDurationS + iProgramFastRampDegrees * fastSecondPerDegree;
